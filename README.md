...
The script uses `ControlSetText` to write the average. If the JTAlert Log Fields layout changes, confirm which control contains the PWR field. Update the `Edit18` target in `WritePowerToJTAlert()` if necessary, then retest with the Log Fields window visible.  
